...
namespace CinsAptServer
{
    internal static class Program
    {
        class CinsAptServer : Form
        {
            ...
            public static List<Socket> socketList;//to store all connected sockets
            public String weatherinfo = "";//to store weather info.
            Thread curthread;
            Thread cardthread;
            ...
            public CinsAptServer()
            {
                ...
                socketList = new List<Socket>();//a collection of active sockets to send messages to all.
            ...
            async void weather()
            {
                while (true)
                {
                    try
                    {
                        string endpoint = "http://api.openweathermap.org/data/2.5/weather?q=Izmir,tr&APPID=APIKEYISDELETEDFORGITHUB";
            ...
            async void currency()
            {
                while (true)
                {
                    try
                    {
                        string endpoint = "https://v6.exchangerate-api.com/v6/APIKEYISDELETEDFORGITHUB/latest/TRY";
                        HttpClient client = new HttpClient();
                        HttpResponseMessage response = await client.GetAsync(endpoint);
                        string responseContent = await response.Content.ReadAsStringAsync();
                        dynamic data = JsonConvert.DeserializeObject(responseContent);
                        double dolar = data.conversion_rates.USD;
                        dolar = 1 / dolar;
                        double gbp = data.conversion_rates.GBP;
                        gbp = 1 / gbp;
                        double eur = data.conversion_rates.EUR;
                        eur = 1 / eur;
                        //our api doesnt provide a momentary info about currencies, it updates in minutes so we faked like we have momentary info about currencies like on the TV's.
                        for (int i = 0; i <= 100; i++)
                        {
                            Random rand = new Random();
